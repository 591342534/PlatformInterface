...
// Returns:     None
//
CXEventLog::CXEventLog(LPCTSTR lpszApp, LPCTSTR lpszEventMessageDll): m_hEventLog(NULL)
{
    // open event log
                                ...
                                LPCTSTR lpszEventMessageDll)
{
    ...
    std::wstring strKey(L"SYSTEM\\CurrentControlSet\\Services\\Eventlog\\Application\\");
    strKey += lpszApp;

    // open the registry event source key
    DWORD dwResult = 0;
    HKEY hKey = NULL;
    LONG lRet = ::RegCreateKeyEx(HKEY_LOCAL_MACHINE, strKey.c_str(), 0, NULL,
    ...
    if (lRet == ERROR_SUCCESS)
    {
        ...
        ::RegSetValueEx(hKey, _T("TypesSupported"), 0, REG_DWORD, (const BYTE *) &dwSupportedTypes, sizeof(DWORD));
...
void CXEventLog::SetAppName(LPCTSTR lpszApp)
{
    ...
    if (!m_pszAppName)
    {
        m_pszAppName = std::make_unique<TCHAR[]>(_MAX_PATH * 2);

    }

    if (m_pszAppName)
    {
        memset(m_pszAppName.get(), 0, _MAX_PATH*2*sizeof(TCHAR));
        wcsncpy(m_pszAppName.get(), lpszApp, _MAX_PATH*2-2); // Flawfinder: ignore
    }
}
